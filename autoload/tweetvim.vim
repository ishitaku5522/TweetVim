...
function! tweetvim#userstream(bang, ...)
  ...
  let s:cursormoved_count = 0
  augroup tweetvim-userstream
    autocmd!
    autocmd! CursorMoved,CursorMovedI * call s:cursormoved_counter()
...
function! s:cursormoved_counter()
  if s:cursormoved_count > 20
    call s:receive_userstream()
    let s:cursormoved_count = 0
  else
    let s:cursormoved_count += 1
  endif
endfunction
...
function! s:receive_userstream()
  ...
  if &filetype != 'tweetvim' && mode() =~# '[niR]'
    let tweetvim_winidlist = win_findbuf(bufnr(g:tweetvim_buffer_name))
    let temp_cur_winid = win_getid()

    let tweetvim_sametab_idx = -1

    if tweetvim_winidlist != []
      for i in range(len(tweetvim_winidlist))
        if win_id2tabwin(temp_cur_winid)[0] == win_id2tabwin(tweetvim_winidlist[i])[0]
          let tweetvim_sametab_idx = i
        endif
      endfor
    endif

    if tweetvim_sametab_idx >= 0
      
      " Go alternate buffer and get its winid
      exe "normal! \<C-w>p"
      let temp_prev_winid = win_getid()

      " Go Tweetvim buffer
      call win_gotoid(tweetvim_winidlist[tweetvim_sametab_idx])
      ...
      if get(b:, 'tweetvim_method', '') != 'userstream'
        ...
        call win_gotoid(temp_prev_winid)
        call win_gotoid(temp_cur_winid)
        return
      endif
    else
      return
    endif
  elseif get(b:, 'tweetvim_method', '') != 'userstream'
    return
  endif
